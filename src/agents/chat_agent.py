...
class ChatAgent:
    def __init__(self):
        ...
        try:
            ...
            # Embedding model not available (e.g., cache/download issue). Disable RAG gracefully.
    ...
    def initialize_vector_store(self, text_content):
        """Create vector store from text content. If embeddings are unavailable, return a no-op store."""
        if not text_content or text_content.strip() == "":
            text_content = "No report context available."

        if not self.rag_enabled or self.embeddings is None:
            class _DummyRetriever:
                def get_relevant_documents(self_inner, query):
                    return []

            class _DummyVectorStore:
                def as_retriever(self_inner, search_kwargs=None):
                    return _DummyRetriever()

            return _DummyVectorStore()

        texts = self.text_splitter.split_text(text_content) or [text_content]
        try:
            return FAISS.from_texts(texts, self.embeddings)
        except Exception:
            # If FAISS or embeddings fail at runtime, fall back to no-op retriever
            class _DummyRetriever:
                def get_relevant_documents(self_inner, query):
                    return []

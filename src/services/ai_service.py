...
def init_analysis_state():
    ...
    if "chat_agent" not in st.session_state:
        try:
            ...
            if "GROQ_API_KEY" not in st.secrets and "DEEPSEEK_API_KEY" not in st.secrets:
                st.session_state.chat_agent = None
                st.session_state.chat_agent_error = "No chat API key found. Add GROQ_API_KEY or DEEPSEEK_API_KEY to .streamlit/secrets.toml"
            else:
...
def get_chat_response(query, context_text, chat_history):
    ...
    if st.session_state.chat_agent is None:
        error_msg = st.session_state.get(
            "chat_agent_error",
            "Chat functionality is currently unavailable. Please set GROQ_API_KEY or DEEPSEEK_API_KEY in .streamlit/secrets.toml",
        )

...
def handle_form_submission(patient_name, age, gender, pdf_contents):
    ...
    with st.spinner("Analyzing report..."):
        ...
        # Save user message locally and proceed with analysis
        session_id = st.session_state.current_session["id"]
        st.session_state.local_messages.setdefault(session_id, []).append(
            {
                "role": "user",
                "content": f"Analyzing report for patient: {patient_name}",
                "created_at": st.session_state.get("last_analysis", None) or "",
            }
        )
        ...
        if result["success"]:
            ...
            st.session_state.local_messages.setdefault(session_id, []).append(
                {"role": "system", "content": report_metadata}
            )
